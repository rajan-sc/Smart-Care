...
const PatientDetailContent: React.FC<{ id: string }> = ({ id }) => {
  ...
  const { data: vitals } = useSuspenseQuery({
    queryKey: ['caregiver-patient-vitals', id],
    queryFn: () => caregiverApi.getPatientVitals(id).then((r) => r.data.data),
    staleTime: 60 * 1000,
  });

  const { data: medications } = useSuspenseQuery({
    queryKey: ['caregiver-patient-medications', id],
    queryFn: () => caregiverApi.getPatientMedications(id).then((r) => r.data.data),
    staleTime: 5 * 60 * 1000,
  });

  const { data: appointments } = useSuspenseQuery({
    queryKey: ['caregiver-patient-appointments', id],
    queryFn: () => caregiverApi.getPatientAppointments(id).then((r) => r.data.data),
    staleTime: 5 * 60 * 1000,
  });

  const { data: records } = useSuspenseQuery({
    queryKey: ['caregiver-patient-records', id],
    queryFn: () => caregiverApi.getPatientRecords(id).then((r) => r.data.data),
    staleTime: 5 * 60 * 1000,
  });

  const patientLink = patients?.find((p: any) => p.patientId === id);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 animate-fade-in">
        <div>
          <h1 className="text-4xl font-display font-semibold tracking-tight text-forest-ink mb-2 flex items-center gap-4">
            ...
            {patientLink && (
              <span className="px-3 py-1 rounded-badges text-xs font-bold uppercase tracking-widest bg-mint-veil text-forest-ink border border-forest-ink/10 shadow-sm">
                {patientLink.relationship}
              </span>
            )}
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="card p-0 overflow-hidden bg-linen-white border border-hairline-gray rounded-cards flex flex-col">
            <div className="p-6 border-b border-hairline-gray bg-mint-veil/20 flex items-center gap-3">
              ...
              <h3 className="text-lg font-bold tracking-tight text-forest-ink">Appointments</h3>
            </div>
            <div className="divide-y divide-hairline-gray bg-linen-white max-h-[400px] overflow-y-auto custom-scrollbar">
              {appointments && appointments.length > 0 ? (
                appointments.map((appt: any) => (
                  <div key={appt.id} className="p-5 hover:bg-mint-veil/10 transition-colors">
                    <div className="flex justify-between items-start mb-1">
                      <p className="font-bold text-forest-ink tracking-tight">Dr. {appt.doctor?.firstName} {appt.doctor?.lastName}</p>
                      <span className={`px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-widest border ${appt.status === 'CONFIRMED' || appt.status === 'COMPLETED' ? 'bg-mint-veil text-forest-ink border-forest-ink/10' : 'bg-linen-white text-graphite border-hairline-gray'}`}>{appt.status}</span>
                    </div>
                    <p className="text-sm font-medium text-graphite mb-1 flex items-center gap-2">
                      <span className="font-mono">{format(parseISO(appt.scheduledDate), 'MMM d, yyyy')}</span>
                      <span>•</span>
                      <span className="font-mono">{format(parseISO(appt.scheduledDate), 'h:mm a')}</span>
                    </p>
                    {appt.notes && (
                      <p className="text-xs font-medium text-forest-ink mt-2 bg-mist-blue/10 border border-hairline-gray p-2.5 rounded-lg">
                        {appt.notes}
                      </p>
                    )}
                  </div>
